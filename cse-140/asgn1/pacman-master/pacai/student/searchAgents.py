...
def DEFAULT_COST_FUNCTION(x): return 1
...
class CornersProblem(SearchProblem):
    ...
    # Initalizes the state of the search problem
    def startingState(self):
        # Position and corners visited.
        startState = (self.startingPosition, (False, False, False, False))
        return startState

    def isGoal(self, gameState):
        cornerStates = gameState[1]

        # Testing
        testMsg = "\n cornerStates is of type: " + \
            str(type(cornerStates)) + "|| cornerStates is: " + \
            str(cornerStates) + "\n"
        print(gameState)
        print(testMsg)

        for cornerVisited in cornerStates:
            if cornerVisited == False:  # If there is a corner we have not visited yet then we arent finished
                return False
        return True  # If we have visited every corner then we are finished

    def successorStates(self, gameState):
        """
        Returns successor states, the actions they require, and a constant cost of 1.
        """

        successors = []
        visitedCorners = gameState[1]  # Tuple of visited corners

        # For each possible action we can perform
        for action in Directions.CARDINAL:
            x, y = gameState[0]
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)

            # If we arent going to hit a wall on our next move
            if (not self.walls[nextx][nexty]):
                nextPos = (nextx, nexty)
                cost = self.costFn(nextPos)  # Should be a constant 1

                # We update the "tuple" of booleans of corners that we have visited
                visitCheck = list(gameState[1])
                for i in range(len(self.corners)):
                    if nextPos == self.corners[i] and visitCheck[i] == False:
                        visitCheck[i] = True

                    newVisitedCorners = tuple(visitCheck)
                # print("successorsAppend: " + str((nextPos, newVisitedCorners, action, cost)))
                successors.append((nextPos, newVisitedCorners, action, cost))
            self._numExpanded += 1
        # print("Successors list: " + str(successors))
        return successors
...
def cornersHeuristic(state, problem):
    ...
    # *** Your Code Here ***
    cornersCoords = problem.corners
    currentPos = state[0]
    cornersVisited = state[1]

    bestValue = -1
    for i in range(len(cornersCoords)):
        if not cornersVisited[i]:  # If we havent visited that node
            cX, cY = cornersVisited[i]
            hN = distance.manhattan(currentPos, (cX, cY)) + \
                distance.euclidean(currentPos, (cX, cY))

        if hN > bestValue:  # If we get a better heurisitc
            bestValue = hN
