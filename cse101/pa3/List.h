...
int front(List L); //Returns the front element (PreCond: length() > 0)

int back(List L); //Returns the back element (PreCond: length() > 0)

int get(List L); //Returns the current element (PreCond: length() > 0, index() >= 0)

bool equals(List A, List B); //Returns true iff Lists A and B are in the same state. Returning false otherwise

//Manipulation Procedures

void clear(List L); //Resets L to its original empty state

void set(List L, int x); //Overwrites the cursor element with x (PreCond: length() > 0, index() >= 0)
...
void prepend(List L, int x); //inserts new element into L. If L is non-empty insertion takes place before front element

void append(List L, int x); //inserts new element into L. If L is non-empty insertion takes place after back element

void insertBefore(List L, int x); //insert new element before cursor (PreCond: length() > 0, index() >= 0)

void insertAfter(List L, int x); //insert new element after cursor (PreCond: length() > 0, index() >= 0)
...
List copyList(List L); //Returns a new List representing the same integer sequence as L. The cursor in the new list in undefined, regardless of the state of the cursor in L. The state of L is unchanged.
